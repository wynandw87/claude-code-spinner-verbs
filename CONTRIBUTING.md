...
2. Run `python3 scripts/check_counts.py --fix`. It updates the category count, the table-of-contents entry, and the totals in the intro.
...
1. Add a section in alphabetical order among the existing ones. The count in the heading can be `(0)`; the script corrects it. Keep `| Verb |` as the table header.
   ...
   An optional one-line description can go between the heading and the table.

2. Run `python3 scripts/check_counts.py --fix`. It corrects the counts and adds the table-of-contents entry in alphabetical order. Categories whose name ends in "Phrases" go under **Spinner Phrases**, everything else under **Spinner Verbs**. For a longer-phrase category without "Phrases" in its name, use `--fix --phrases`.
